...
import { HistorialTraspaso } from '../components/HistorialTraspaso'
...
export function TraspasosPage() {
  ...
  const [origenId, setOrigenId] = useState<number | null>(user?.idAlmacen ?? null)
  ...
  const [sessionItems, setSessionItems] = useState<
    { descripcion: string; cantidad: number; unidad: string | null; destino: string }[]
  >([])
  ...
  const origenNombre = almacenes.find((a) => a.id === origenId)?.almacen
  const destinoNombre = destinos.find((a) => a.id === destinoId)?.almacen
  const listo = origenId != null && destinoId != null
  ...
  function guardar() {
    ...
    if (origenId == null || destinoId == null) {
      setErr('Selecciona almacen origen y destino.')
      return
    }
    ...
    setErr(null)
    const destino = destinoNombre ?? `Almacen ${destinoId}`
    const payload: GuardarTraspasoPayload = {
      user_id: user.id,
      sucursal_id: sucursalId,
      almaceno_id: origenId,
      almacend_id: destinoId,
    ...
    enqueue({
      kind: 'traspaso',
      payload,
      label: `${selected.descripcion} · ${cant} ${selected.unidad ?? ''} · → ${destino}`,
      createdAt: payload.created_at,
    })
    scanFeedback()
    setSessionCount((n) => n + 1)
    setSessionItems((list) => [
      { descripcion: selected.descripcion, cantidad: cant, unidad: selected.unidad, destino },
      ...list,
    ])
    setMsg(`Traspaso guardado: ${selected.descripcion} (${cant} ${selected.unidad ?? ''})`)
    resetCaptura()
  }
  ...
  return (
    <Screen>
      <AppBar
        title="Traspasos"
        subtitle={origenNombre && destinoNombre ? `${origenNombre} → ${destinoNombre}` : undefined}
        right={
          <button
            onClick={() => navigate('/')}
      ...
      <div className="grid grid-cols-2 gap-3 border-b border-slate-200 bg-white px-4 py-3">
        <label className="block">
          ...
          >
            ...
            {almacenes.length === 0 && origenId != null && (
              <option value={origenId}>Almacen {origenId}</option>
            )}
          </select>
        </label>
        <label className="block">
      ...
      <div className="flex-1 space-y-4 px-4 py-4">
        ...
        <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-card">
          <div className="text-sm">
            <span className="text-slate-500">Traspasos en esta sesion: </span>
            ...
            {sessionItems.slice(0, 8).map((it, i) => (
              <li key={i} className="flex items-center justify-between px-4 py-2">
                <span className="min-w-0 truncate text-sm text-slate-700">
                  {it.descripcion} <span className="text-slate-400">→ {it.destino}</span>
                </span>
      ...
      <Sheet open={selected !== null} onClose={resetCaptura} title="Cantidad a traspasar">
        {selected && (
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="font-semibold text-slate-900">{selected.descripcion}</p>
              <p className="text-xs text-slate-500">
                PLU {selected.codigo} · {selected.unidad ?? 's/u'} · {origenNombre ?? 'Origen'} →{' '}
                {destinoNombre ?? 'Destino'}
              </p>
            </div>
            <TextField
              ref={cantidadRef}
              label={`Cantidad a traspasar (${selected.unidad ?? 'unid.'})`}
              type="number"
              inputMode="decimal"
              min={0}
              step="any"
      ...
      <Sheet open={histOpen} onClose={() => setHistOpen(false)} title="Historial">
        {origenId != null ? (
          <HistorialTraspaso sucursalId={sucursalId} almacenOrigenId={origenId} userId={user.id} />
        ) : (
          <p className="py-6 text-center text-sm text-slate-500">Selecciona el almacen origen.</p>
        )}
      </Sheet>
    </Screen>
  )
